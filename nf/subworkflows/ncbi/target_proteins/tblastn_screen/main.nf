...
workflow tblastn_wnode {
    ...
    main:
        ...
        (jobs, lines_per_file) = gpx_qsubmit(genome_asn, proteins_asn, blastdb, seqids, gpx_qsubmit_params)
        jobs = jobs.flatten()
...
process gpx_qsubmit {
    ...
    script:
        njobs=split_count
    """
    echo $seqids > gilist.mft
    head -100 $seqids > short_list 
    echo 'short_list' > ./gilist.mft
    echo $blastdb/RepeatMasker.lib > blastdb.mft
    ...
    gpx_qsubmit $params -ids-manifest gilist.mft -o jobs -nogenbank -asn-cache ./asncache/ -db-manifest blastdb.mft   -batch-size 1
    ### for other tblastn -max-batch-length 10000  -affinity subject
...
##-db_soft_mask 'NCBI-GPipe-Softmask' 
    mkdir -p blast
    cat interim/* > blast/tblastn_wnode.${task.index}.gpx-job.asn
    rm -rf interim
    """
    stub:
    """
    mkdir -p blast
    touch blast/tblastn_wnode.${task.index}.gpx-job.asn
    """
}
